...
// ==========================
// BRAND + LOGO (refreshed professional mark)
// ==========================
const BRAND = { dark: "#1f160f", lime: "#b6e300" };
const LOGO_URL = null; // set to "/logokrew.png" after deploying and placing the file in /public

// Minimal vector mark for demo
const LogoMark = ({ size = 24 }) => (
  <svg width={size} height={size} viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="shrink-0">
    <path d="M10 26 H4" stroke={BRAND.lime} strokeWidth="4" strokeLinecap="round"/>
    <path d="M14 34 H2" stroke={BRAND.lime} strokeWidth="4" strokeLinecap="round"/>
    <rect x="6" y="12" width="42" height="28" rx="6" fill={BRAND.lime} />
    <rect x="48" y="18" width="10" height="10" rx="2" fill={BRAND.lime} />
    <circle cx="52" cy="44" r="5" fill={BRAND.lime} stroke={BRAND.dark} strokeWidth="4"/>
    <path d="M14 36 V16 h6 l8 10 V16 h6 v20 h-6 l-8-10 v10h-6z" fill={BRAND.dark}/>
    <path d="M40 16 v8 l-4 4 4 4 v8 h-6 v-8 l-4-4 4-4 v-8h6z" fill={BRAND.dark}/>
  </svg>
);

function buildLogoSVG() {
  return `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><defs><style><![CDATA[ .d{fill:${BRAND.dark}} .l{fill:${BRAND.lime}} ]]></style></defs><path d=\\"M10 26H4\\" stroke=\\"${BRAND.lime}\\" stroke-width=\\"4\\" stroke-linecap=\\"round\\"/><path d=\\"M14 34H2\\" stroke=\\"${BRAND.lime}\\" stroke-width=\\"4\\" stroke-linecap=\\"round\\"/><rect x=\\"6\\" y=\\"12\\" width=\\"42\\" height=\\"28\\" rx=\\"6\\" class=\\"l\\"/><rect x=\\"48\\" y=\\"18\\" width=\\"10\\" height=\\"10\\" rx=\\"2\\" class=\\"l\\"/><circle cx=\\"52\\" cy=\\"44\\" r=\\"5\\" class=\\"l\\" stroke=\\"${BRAND.dark}\\" stroke-width=\\"4\\"/><path d=\\"M14 36V16h6l8 10V16h6v20h-6l-8-10v10h-6z\\" class=\\"d\\"/><path d=\\"M40 16v8l-4 4 4 4v8h-6v-8l-4-4 4-4v-8h6z\\" class=\\"d\\"/></svg>`;
}
function downloadLogoSVG(){
  ...
  a.href = url;
  a.download = 'neighborhood-krew-logo.svg';
  a.click();
...
const Meta = () => (
  <>
    ...
    <meta name="description" content="Professional, reliable movers serving Greater Philadelphia & New Jersey. Local & long‑distance moves, packing, junk removal, and commercial freight." />
...
// ==========================
// LOCAL STORAGE (LEADS / NEWSLETTER)
// ==========================
...
// Small helper so we can unit-test the newline behavior
...
// --- Light integration + gallery helpers ---
// Default gallery URLs for production (served from /public/gallery)
const DEFAULT_GALLERY_URLS = [
  "/gallery/krew1-opt.jpg",
  "/gallery/krew2-opt.jpg",
  "/gallery/krew3-opt.jpg",
  "/gallery/krew4-opt.jpg",
  "/gallery/krew5-opt.jpg",
  "/gallery/krew6-opt.jpg",
];
const GALLERY_KEY = "nkrew_gallery_objs"; // stores [{src, alt, ts}]
const WEBHOOK_KEY = "nkrew_webhook"; // optional webhook URL to connect Mailchimp/Brevo/Zapier
...
function ensureDefaultGallery() {
  const current = loadGallery();
  if (current && current.length > 0) return current;
  const seeded = DEFAULT_GALLERY_URLS.map((src, i) => ({ src, alt: `Krew job ${i+1}`, ts: Date.now() }));
  saveGallery(seeded);
  return seeded;
}
    ...
    <div className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur">
      <nav className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <a href="#top" className="flex items-center gap-2 font-semibold text-lg">
          {LOGO_URL ? (
            <img src={LOGO_URL} alt="Neighborhood Krew Inc logo" className="h-6 w-6" />
          ) : (
            <LogoMark size={24} />
          )}
  ...
  <section id="top" className="relative overflow-hidden" style={{backgroundColor: BRAND.dark}}>
    <div className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
      <div>
        ...
        <p className="mt-4 text-white/80 text-lg">Local & long‑distance moving, packing, junk removal, and freight. Serving Greater Philadelphia & New Jersey.</p>
...
const Areas = () => (
  <section id="areas" className="py-16 md:py-24 bg-gray-50">
    <div className="max-w-6xl mx-auto px-4">
      ...
      <p className="mt-2 text-gray-600 max-w-2xl">Proudly serving Greater Philadelphia and New Jersey, plus long‑distance routes across the East Coast.</p>
      <div className="mt-8 grid md:grid-cols-3 gap-6">
        {["Philadelphia","Princeton, NJ","Greater New Jersey","Montgomery County, PA","Delaware County, PA","Bucks County, PA"].map((t)=> (
...
const Pricing = ({ onOpenQuiz }) => (
  <section id="pricing" className="py-16 md:py-24 bg-white">
    <div className="max-w-6xl mx-auto px-4">
      ...
      <div className="mt-8 grid md:grid-cols-3 gap-6">
        {[{
          title:"2 Movers + Truck", price:"$150/hr", features:["Best for studios","2‑hour minimum","Basic protection"],
        },{
...
// ==========================
// GALLERY (URL + local uploads)
// ==========================
const Gallery = () => {
  const [items, setItems] = useState(() => ensureDefaultGallery()); // [{src, alt, ts}]
  ...
  const resetToDefaults = () => { const seeded = DEFAULT_GALLERY_URLS.map((src, i) => ({ src, alt: `Krew job ${i+1}`, ts: Date.now() })); setItems(seeded); saveGallery(seeded); };
  return (
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <form onSubmit={onAddUrl} className="mt-4 flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button type="button" variant="outline" onClick={resetToDefaults}>Load Demo Photos</Button>
          </div>
        </form>
    ...
    <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-5 gap-8">
      <div className="md:col-span-3">
        ...
        <div className="mt-6 space-y-3 text-gray-700">
          <p className="flex items-center gap-2"><Phone className="h-4 w-4"/> (215) 531‑0907</p>
          <p className="flex items-center gap-2"><Mail className="h-4 w-4"/> Neighborhoodkrew@gmail.com</p>
          <p className="flex items-center gap-2"><MapPin className="h-4 w-4"/> Philadelphia, PA & New Jersey</p>
        </div>
      </div>
...
// ==========================
// ADMIN PANEL (with webhook setting)
// ==========================
const AdminPanel = () => {
  const [open, setOpen] = useState(false);
          ...
          <CardContent className="space-y-4">
            ...
            <div className="border-t pt-3">
              ...
              <div className="flex gap-2 mt-4">
                <Button variant="outline" onClick={downloadLogoSVG}>Download new logo (SVG)</Button>
              </div>
            </div>
          </CardContent>
        </Card>
...
const TextUsFAB = () => (
  <a
    href="sms:+12155310907"
    className="fixed md:hidden bottom-4 right-4 z-50 rounded-full shadow-2xl px-5 py-3 font-medium"
    style={{ backgroundColor: BRAND.lime, color: "#111" }}
    aria-label="Text Neighborhood Krew"
  >
    Text Us
  </a>
);
...
async function runSelfTests() {
  try {
    console.assert(typeof scoreLead === "function", "scoreLead should be a function");
    console.assert(scoreLead({ fromZip:"19000", toZip:"28000", size:"Single Family Home", services:{packing:true, junk:false, assembly:true, longCarry:false, freight:false}, timing:"ASAP" }) >= 4, "scoreLead basic test");

    // Gallery storage test
    const before = loadGallery().length; saveGallery([{src:'data:image/png;base64,AA', alt:'t', ts:Date.now()}]); const after = loadGallery().length; console.assert(after >= 1, 'gallery save/load'); saveGallery([]);

    // CSV newline test
    const csvTest = csvStringFromRows(["a,b", "c,d"]);
    console.assert(csvTest === "a,b\\nc,d", "csvStringFromRows should join with \\\\n");

    // Newsletter dedupe test (non-destructive)
    const prev = localStorage.getItem("nkrew_newsletter");
    localStorage.setItem("nkrew_newsletter", "[]");
    await subscribeAndSendPromo("test@example.com");
    await subscribeAndSendPromo("test@example.com");
    const list = JSON.parse(localStorage.getItem("nkrew_newsletter") || "[]");
    const count = list.filter(x => x.email === "test@example.com").length;
    console.assert(count === 1, "newsletter should dedupe same email");
    if (prev !== null) localStorage.setItem("nkrew_newsletter", prev);
    else localStorage.removeItem("nkrew_newsletter");

    // Existence tests
    console.assert(typeof TextUsFAB === "function", "TextUsFAB should be defined");
    console.assert(typeof ensureDefaultGallery === "function", "ensureDefaultGallery exists");

    console.info("Neighborhood Krew: self-tests passed");
